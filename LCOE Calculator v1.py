...
def find_cheapest(
    ...
    combine = np.column_stack((j_array,costs_array))
    print(combine)
...
TO DO:
    ...
    > Think about how the code can be simplified - one for loop?
    > use 2D array with all the data/panda dataframes?
    """
...
def incremental_build(
    ...
    for sub_array in inc_array:         # For loop calculates the pv size steps
        ...
        total_pv_array = np.append(total_pv_array, (pv_size_recursive(initial_daily_demand, last_val, capacity_factor, demand_increase, degradation_rate)))
    ...
    print(inc_array)
    print(total_pv_array)
    print(pv_build_array)
...
def pv_size_recursive(initial_daily_demand, operating_period, capacity_factor, demand_increase, degradation_rate):

    annual_demand = (initial_daily_demand * 365) #converts daily demand to annual
    def pv_deg_incdem(demand, degradation, capacity_factor):

        hourly = (demand/8760) # assumes demand is the same across every hour
        pv = (hourly/capacity_factor) / (1-degradation)  # calculates the pv capacity needed based
        return pv
    ...
    newpv = pv_deg_incdem(increased_demand, degradedpv, capacity_factor) # Calls function that
...
def energy_output(initial_daily_demand, operating_period, demand_increase, discount_rate):
    ...
    for year in range(0, operating_period): #runs the for-loop for 0 - end of project life
        total_generation += (annual_demand * (demand_increase ** year)) / ((1 + discount_rate) ** year) # Adds to the total the increased and discounted demand
    return float(total_generation) # Returns total figure to main function

# %%
# ==========================================
# Callable function for end of period demand
# ==========================================

def dem_end_period(annual_demand, period, deminc):
    new_demand = annual_demand * (deminc ** period)
    return new_demand
...
# ========================================
def pv_degradation_percent(degradation, period):
